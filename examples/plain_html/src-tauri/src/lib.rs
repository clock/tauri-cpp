...
fn smoke_test() -> Result<String, String> {
    let sync_result = invoke_sync_demo("sync_echo", "{\"mode\":\"smoke\"}");
    let system_result = invoke_sync_demo("system_info", "{}");

    serde_json::to_string_pretty(&serde_json::json!({
        "sync_ok": sync_result.ok,
        "system_ok": system_result.ok,
        "bridge": system_result.payload_json
    }))
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if env::args().any(|arg| arg == "--smoke-test") {
                let result_path = PathBuf::from("smoke-test-result.json");
                let payload = smoke_test()?;
                window.hide().map_err(|error| error.to_string())?;
                fs::write(result_path, payload).map_err(|error| error.to_string())?;
                app.handle().exit(0);
            }
        ...
        .expect("failed to run plain_html app");
}
